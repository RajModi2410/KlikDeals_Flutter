...
class ChangePasswordForm extends StatefulWidget {
  ...
  @override
  State<ChangePasswordForm> createState() => _ChangePasswordFormState(this.callback);
}
      ...
      BlocListener<ApiBlocBloc, ApiBlocState>(
          listener: (context, state) {
            if (state is ChangePasswordErrorState) {
              String error = "";
              if (state.changePasswordResponse.errorMessage.generalError.length > 0) {
                error = state.changePasswordResponse.errorMessage.generalError.first;
                print("We got the error in LOGIN VALUE::$error");
              } else if (state.changePasswordResponse.errorMessage.userError.length >
                  0) {
                // error = state.loginResponse.errorMessage.user_error.first;
                this.loginBloc.emailChanged(ErrorGen(
                    isError: true,
                    value: state.changePasswordResponse.errorMessage.userError.first));
              ...
              token = state.changePasswordResponse.response.toString();
              ...
              _goToHomePage();
                            ...
                            child: Padding(
                              ...
                              child: TextFormField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintText:
                                      AppLocalizations.of(context).translate("label_newpassword"),
                                ),
                              ),
                            ),
                          ),
                  ...
                  void validateAndSubmit() {
                    ...
                    if (validateAndSave()) {
                      try {
                        lastEvent = ChangePasswordEvent(_email, _password);
                        auth.add(lastEvent);
                        setState(() {});
                      } catch (e) {
                        print('Error: $e');
